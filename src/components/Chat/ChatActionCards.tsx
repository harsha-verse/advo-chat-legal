...
import {
  Star, MapPin, Scale, FileText, BookOpen, ChevronRight,
} from 'lucide-react';
import {
  detectContext,
  matchLawyers,
  ...
  return (
    <div className="space-y-3 mt-3">
      ...
      <div className="bg-primary/5 rounded-lg p-3 border border-primary/10">
        <p className="text-xs font-semibold text-primary mb-2">⚡ Quick Actions</p>
        <div className="flex flex-wrap gap-2">
          <Button
            size="sm"
            variant="default"
            className="text-xs h-7 gap-1"
            onClick={() => navigate(`/lawyers?category=${ctx.categories[0]}`)}
          >
            <Scale className="h-3 w-3" /> Consult a Lawyer
          </Button>
          {templates.length > 0 && (
            <Button
              size="sm"
              variant="outline"
              className="text-xs h-7 gap-1"
              onClick={() => navigate(`/templates?category=${ctx.categories[0]}`)}
            >
          ...
          >
            <BookOpen className="h-3 w-3" /> {guide.label}
          </Button>
        </div>
      </div>
      ...
      {ctx.needsLawyer && lawyers.length > 0 && (
        <div className="bg-card rounded-lg p-3 border">
          <p className="text-xs font-semibold mb-2">
            {ctx.isComplex
              ? '⚠️ This seems complex. Consider professional help:'
          ...
          <div className="space-y-2">
            {lawyers.map((lawyer) => (
              <LawyerMiniCard
                key={lawyer.id}
                lawyer={lawyer}
                onClick={() => navigate(`/lawyers?category=${lawyer.category}`)}
              />
            ))}
          </div>
        </div>
      ...
      {templates.length > 0 && (
        <div className="bg-card rounded-lg p-3 border">
          <p className="text-xs font-semibold mb-2">📄 Helpful Documents</p>
          <div className="space-y-1.5">
            {templates.map((tpl) => (
              <TemplateMiniCard
                key={tpl.id}
                template={tpl}
                onClick={() => navigate(`/templates?category=${tpl.category}`)}
              />
            ))}
          </div>
        </div>
...
const LawyerMiniCard: React.FC<{ lawyer: MatchedLawyer; onClick: () => void }> = ({
  lawyer,
  onClick,
}) => (
  <button
...
const TemplateMiniCard: React.FC<{ template: MatchedTemplate; onClick: () => void }> = ({
  template,
  onClick,
}) => (
  <button
